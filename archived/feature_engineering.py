...
- it computes advanced technical indicators and Alpha158-inspired features,
  ...
  and creates target cols for multi-day prediction:
...
Note: If the dataset contains multiple stocks ( "company" column), processing is done per company.
"""
...
def feature_engineering_pipeline(df):
    ...
    # next_day_close: close shifted by -1
    # day_after_next_close: close shifted by -2
    # future_5day_close: close shifted by -5
    df["next_day_close"] = df["close"].shift(-1)
    df["day_after_next_close"] = df["close"].shift(-2)
    df["future_5day_close"] = df["close"].shift(-5)
    
    # Replace infinite values and drop NaNs before scaling
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    df.dropna(inplace=True)
    
    # Add this check: if the DataFrame is empty, return it immediately
    if df.empty:
        print("Warning: Group is empty after dropping NaNs. Skipping scaling.")
        return df
...
def main():
    ...
    # If the dataset contains multiple stocks (i.e. a "company" column), process each separately:
    if "company" in df.columns:
        # use swifter to parallelize the groupby apply for scalability
        engineered_df = df.swifter.groupby("company", group_keys=False).apply(feature_engineering_pipeline)
    else:
